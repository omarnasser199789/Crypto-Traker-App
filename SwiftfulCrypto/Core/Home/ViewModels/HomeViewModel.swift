...
class HomeViewModel: ObservableObject{
    ...
    enum SortOption {
        case rank, reankReversed, holdings, holdingsReversed, price, priceReversed
    }
    ...
    func addSubscribers(){
        ...
        // update the marletData
        marketDataService.$marketData
            .combineLatest($portfolioCoins)
            .map(mapGlobalMarketData)
        
            .sink{ [weak self] (returedStats) in
                self?.statistics = returedStats
    ...
    func reloadData(){
        ...
        HapticManager.notificationOccurred(type: .success)
    }
    ...
    private func sortCoins(sort: SortOption, coins: inout [CoinModel]) {
        ...
        case .reankReversed, .holdingsReversed:
             coins.sort(by: {$0.rank > $1.rank})
        case .price:
    ...
    private func sortPortfolioCoinsIfNeeded(coins:[CoinModel]) -> [CoinModel] {
        // will only sort by holding or reversedholdins if needed
        switch sortOption {
        case .holdings:
